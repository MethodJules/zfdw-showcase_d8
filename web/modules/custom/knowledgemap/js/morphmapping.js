...
Indeko.MorphBox.convertMorphsearch = function() {
  ...
  $('#sidebar_first').find('.block:not(#block-hervorgehobenesformularsuchepage-1)').hide(); // hide others blocks in forst sidebar
  ...
  Indeko.MorphBox.elemFulltext.unbind().keyup(Indeko.MorphBox.getSelectedValuesFromMorphBox);  // keyuplistener for fulltext field
	...
	var htmlDirectUrl = '<div class="form-item form-type-textfield direct-url">' +
		...
		'<div><b>OR<b></div>';
  $('#edit-search-api-fulltext').before(htmlDirectUrl);
  $('#direct-url').keyup(Indeko.MorphBox.getSelectedValuesFromMorphBox);

  // TODO internal url prototype
  var elemInternalUrl = $('#edit-field-internal-reference-0-target-id');
  if (elemInternalUrl.length > 0) {
    ...
    Drupal.behaviors.morphmapping = {
      attach: function(context, settings) {
        $('#edit-field-internal-reference-0-target-id').ajaxSuccess(function(event, xhr, settings, data) {
...
Indeko.ImageMap.hookMapAreas = function () {
    $("map area").click(function () {
    	var elemBlockSearchresults = $('#block-views-block-knowledgemap-search-results');
      ...
      if (elemBlockSearchresults.length) {
        ...
        $('#edit-submit-knowledgemap-search-results').click();
        ...
        return false;
      	...
      	// TODO 2020 if (href.indexOf(Drupal.settings.morphsearch.searchPath) === -1) {
